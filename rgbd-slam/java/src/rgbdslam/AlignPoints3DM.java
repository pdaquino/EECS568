...
/* Modified by John Peterson to include relative weighting between corespondances
 *  Use w to weight the correspondances
 */
public class AlignPoints3DM
{
    ...
    public static double[][] align(List<double[]> a, List<double[]> b, List<Double> W)
    {
        ...
        for (int i = 0; i < n; i++) {
            LinAlg.plusEquals(aCent, LinAlg.scale(a.get(i),W.get(i)));
            LinAlg.plusEquals(bCent, LinAlg.scale(b.get(i),W.get(i)));
        }
        aCent = LinAlg.scale(aCent, 1.0/scaleF);
        bCent = LinAlg.scale(bCent, 1.0/scaleF);

        // Now compute M = \Sig (x'_b) (x'_a)^t
        // XXX black magic in this for loop probably needs to be modified to work with weights
        ...
        // Scale by 1/scaleF for numerical precision in next step
        for (int  i = 0; i < m; i++)
            for (int j = 0; j < m; j++)
                M[i][j] /= scaleF;
    ...
    public static void main(String args[])
    {
        ...
        int dimensions[] = {2,2,2,2,2,3,3,3,3,3};
        ...
        for (int dimIdx = 0; dimIdx < dimensions.length; dimIdx++) { // allow for burn in before we get to the cases we really care about (2,3 d)
            ...
            for (int trial  = 0; trial < ntrials; trial++) {
                ...
                for (int i = 0; i < dim + trial/4; i++) { // make 4 set of each size, starting with the minimum
                    ...
                    weights.add(5.0); // just testing with uniform weight
                }

                tic.tic();
                double H[][] = null;
                if (dim == 2 && dimIdx == dimensions.length -1)
                    H = xytToMatrix33(april.jmat.geom.AlignPoints2D.align(start,end));
                else
                    H = align(start,end,weights);
                double time = tic.toc();
